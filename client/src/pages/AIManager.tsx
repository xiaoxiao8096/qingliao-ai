...
export default function AIManager() {
  ...
  async function testMedia(capability: EndpointCapability) {
    ...
    const testPrompts: Record<EndpointCapability, string> = {
      image: "a tiny blue circle on white background",
      speech: "你好，这是一次连接测试。",
      music: "a short cheerful piano loop",
      video: "a short clip of ocean waves",
    };
    setTestingMedia(previous => ({ ...previous, [capability]: true }));
    setMediaTestState(previous => ({ ...previous, [capability]: { kind: "loading", text: "正在向端点发送一次最小化请求…" } }));
    const result = await testMediaCapability(form, capability, testPrompts[capability]);
    setTestingMedia(previous => ({ ...previous, [capability]: false }));
    if (result.ok) {
      setMediaTestState(previous => ({ ...previous, [capability]: { kind: "success", text: result.message } }));
      toast.success("端点可用。", { description: result.message });
    } else {
      setMediaTestState(previous => ({ ...previous, [capability]: { kind: "error", text: result.message } }));
      toast.error("测试未通过。", { description: result.message });
    }
  }
          ...
          <section className="rounded-[1.75rem] bg-white p-5 shadow-[0_16px_55px_rgba(43,58,72,0.08)] sm:p-8">
            ...
            <form onSubmit={submit} className="mt-7 space-y-5">
              ...
              <details className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                ...
                <p className="mt-2 text-xs leading-5 text-slate-500">图片和聊天不是同一家？没问题。每个能力都可以单独填自己那家的 API Key：<span className="font-semibold text-slate-600">填了就用它，留空才回落到上方聊天那把钥匙</span>。端点地址必须以 https 开头；Vercel 版本会通过受限代理访问，静态版本仍要求服务商允许浏览器跨域。填完点「测试生成」立刻知道配得对不对。所有配置只保存在当前浏览器。</p>
