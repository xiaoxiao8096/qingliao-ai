          ...
          <section className="rounded-[1.75rem] bg-white p-5 shadow-[0_16px_55px_rgba(43,58,72,0.08)] sm:p-8">
            ...
            <form onSubmit={submit} className="mt-7 space-y-5">
              ...
              <details className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                ...
                <div className="mt-3 space-y-3">{(["image", "speech", "music", "video"] as const).map(capability => <div key={capability} className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-xs font-bold text-slate-600">{{ image: "图片生成", speech: "语音合成", music: "音乐生成", video: "视频生成" }[capability]}</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><Input value={form.media?.[capability]?.endpoint ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], endpoint: event.target.value } } }))} placeholder="可选：完整 HTTPS 端点" className="h-9 rounded-lg border-slate-200 bg-slate-50 text-xs" /><Input value={form.media?.[capability]?.model ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], model: event.target.value } } }))} placeholder="可选：专用模型名" className="h-9 rounded-lg border-slate-200 bg-slate-50 text-xs" /></div>{capability === "speech" && <Input value={form.media?.speech?.voice ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, speech: { ...previous.media?.speech, voice: event.target.value } } }))} placeholder="可选：语音名，默认 alloy" className="mt-2 h-9 rounded-lg border-slate-200 bg-slate-50 text-xs" />}<details className="mt-2 rounded-lg bg-slate-50 p-2.5"><summary className="cursor-pointer text-xs font-semibold text-slate-500">高级兼容参数（自定义音乐/视频服务才需要）</summary><div className="mt-2 grid gap-2 sm:grid-cols-2"><select value={form.media?.[capability]?.requestFormat ?? (capability === "video" ? "form" : "json")} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], requestFormat: event.target.value as "json" | "form" } } }))} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs text-slate-600"><option value="json">JSON 请求体</option><option value="form">表单请求体</option></select><Input value={form.media?.[capability]?.resultPath ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], resultPath: event.target.value } } }))} placeholder="可选：结果字段，如 data.0.url" className="h-9 rounded-lg border-slate-200 bg-white text-xs" /></div><div className="mt-2 grid gap-2 sm:grid-cols-2"><Input value={form.media?.[capability]?.resultMimeType ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], resultMimeType: event.target.value } } }))} placeholder="可选：裸 Base64 的 MIME，如 audio/mpeg" className="h-9 rounded-lg border-slate-200 bg-white text-xs" /><span className="self-center text-[10px] leading-4 text-slate-400">二进制或结果链接会自动读取 MIME，无需填写。</span></div><Textarea value={form.media?.[capability]?.requestTemplate ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], requestTemplate: event.target.value } } }))} placeholder={'可选 JSON，如 {"model":"{{model}}","prompt":"{{prompt}}"}'} className="mt-2 min-h-16 rounded-lg border-slate-200 bg-white text-xs" />{(capability === "music" || capability === "video") && <div className="mt-2 grid gap-2 sm:grid-cols-2"><Input value={form.media?.[capability]?.pollEndpoint ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], pollEndpoint: event.target.value } } }))} placeholder="可选：轮询端点，使用 {{id}}" className="h-9 rounded-lg border-slate-200 bg-white text-xs" /><Input value={form.media?.[capability]?.contentEndpoint ?? ""} onChange={event => setForm(previous => ({ ...previous, media: { ...previous.media, [capability]: { ...previous.media?.[capability], contentEndpoint: event.target.value } } }))} placeholder="可选：内容端点，使用 {{id}}" className="h-9 rounded-lg border-slate-200 bg-white text-xs" /></div>}</details></div>)}</div>
              </details>
