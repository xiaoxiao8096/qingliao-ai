...
export default function Home() {
  ...
  async function sendMessage(content: string) {
    ...
    updateStoredConversations(setConversations, previous => appendLocalMessages(previous, conversationId, [userMessage, assistantMessage], conversation!.messages.length === 0 ? initialTitle(content) : undefined));
    setIsStreaming(true);
    try {
      const response = await fetch(modelEndpoint(activeAI.baseUrl), { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${activeAI.apiKey}` }, body: JSON.stringify({ model: activeAI.model, stream: true, messages: [{ role: "system", content: `你是 ${activeAI.name}，一个准确、友善、简洁的助手。` }, ...history, { role: "user", content }] }) });
    ...
    } catch (error) { updateStoredConversations(setConversations, previous => dropEmptyAssistantMessage(previous, conversationId, assistantMessage.id)); toast.error(error instanceof Error ? error.message : "发送失败，请检查 API 设置或网络。"); } finally { setIsStreaming(false); }
  }
