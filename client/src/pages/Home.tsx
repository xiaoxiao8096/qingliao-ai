...
function ThemeAppearancePreview({ appearance, assistantName, userName, expanded = false, onExpand }: { appearance: AIAppearance; assistantName: string; userName: string; expanded?: boolean; onExpand?: () => void }) {
  ...
  const fontSize = expanded ? (appearance.fontScale === "small" ? "15px" : appearance.fontScale === "large" ? "20px" : "17px") : (appearance.fontScale === "small" ? "11px" : appearance.fontScale === "large" ? "15px" : "13px");
  ...
  const fontName = appearance.fontScale === "small" ? "小字" : appearance.fontScale === "large" ? "大字" : "标准字";
  const filterName = appearance.backgroundSaturation === 0 ? "黑白" : (appearance.backgroundBlur ?? 0) > 0 ? `模糊 ${appearance.backgroundBlur ?? 0}px` : (appearance.backgroundGradientOpacity ?? 0) > 0 ? "渐变叠色" : "原始色彩";

  return <div className={`theme-preview-card relative mt-2 overflow-hidden border border-slate-200 bg-slate-100 shadow-inner ${expanded ? "min-h-[31rem] rounded-3xl" : "min-h-[13rem] cursor-zoom-in rounded-xl"}`} aria-label={onExpand ? "主题效果实时预览，点击展开" : "主题效果实时预览"} onClick={onExpand} onKeyDown={event => { if (onExpand && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onExpand(); } }} role={onExpand ? "button" : undefined} tabIndex={onExpand ? 0 : undefined}>
    ...
    <div className={`relative ${expanded ? "min-h-[31rem] p-5 sm:p-7" : "min-h-[13rem] p-3"}`}>
      <div className="flex items-center justify-between"><div className={`flex items-center ${expanded ? "gap-2.5" : "gap-2"}`}><span className={`grid place-items-center rounded-lg font-black text-white shadow-sm ${expanded ? "size-10 text-sm" : "size-7 text-[10px]"}`} style={{ backgroundColor: palette.primary }}>{assistantInitial}</span><span className={`${expanded ? "max-w-48 text-base" : "max-w-32 text-[11px]"} truncate font-bold text-slate-700`}>{assistantName || "当前 AI"}</span></div><span className={`rounded-full border border-white/80 bg-white/75 font-bold ${expanded ? "px-3 py-1 text-xs" : "px-2 py-0.5 text-[9px]"}`} style={{ color: palette.ink }}>{appearance.backgroundImage ? "自定义背景" : "实时预览"}</span></div>
      <div className={`${expanded ? "mt-14 space-y-6" : "mt-5 space-y-3"}`} style={{ fontSize }}><div className={`flex items-end ${expanded ? "gap-3" : "gap-2"}`}><span className={`grid shrink-0 place-items-center rounded-full font-bold text-white ${expanded ? "size-8 text-xs" : "size-5 text-[8px]"}`} style={{ backgroundColor: palette.primary }}>{assistantInitial}</span><span className={`max-w-[74%] bg-white/92 text-slate-600 shadow-sm ${expanded ? "px-4 py-3" : "px-2.5 py-2"}`} style={{ borderRadius: miniPreviewRadius(appearance.bubbleRadius) }}>现在的视觉更有辨识度。</span></div><div className={`flex items-end justify-end ${expanded ? "gap-3" : "gap-2"}`}><span className={`max-w-[70%] text-white shadow-sm ${expanded ? "px-4 py-3" : "px-2.5 py-2"}`} style={{ borderRadius: miniPreviewRadius(appearance.bubbleRadius), backgroundColor: palette.primary }}>这样一眼就能看出来。</span><span className={`grid shrink-0 place-items-center rounded-full bg-slate-700 font-bold text-white ${expanded ? "size-8 text-xs" : "size-5 text-[8px]"}`}>{userInitial}</span></div></div>
      <div className={`absolute flex flex-wrap items-center gap-1.5 rounded-xl border border-white/80 bg-white/80 font-semibold text-slate-600 shadow-sm ${expanded ? "inset-x-5 bottom-5 px-4 py-2.5 text-xs sm:inset-x-7 sm:bottom-7" : "inset-x-3 bottom-3 px-2 py-1.5 text-[9px]"}`}><span className="flex items-center gap-1"><i className="size-1.5 rounded-full" style={{ backgroundColor: palette.primary }} />{textureName}</span><span>· {radiusName}</span><span>· {fontName}</span><span>· {filterName}</span></div>
      {onExpand && <span className="absolute right-3 top-11 grid size-6 place-items-center rounded-lg bg-white/80 text-slate-500 shadow-sm"><Maximize2 className="size-3.5" /></span>}
    </div>
  </div>;
}
...
function ThemeComparisonCard({ theme, before, position, onPositionChange, onApply }: { theme: typeof BUILTIN_AI_THEMES[number]; before: AIAppearance; position: number; onPositionChange: (value: number) => void; onApply: () => void }) {
  ...
  return <div className="group rounded-xl border border-slate-200 bg-white p-1.5 text-center shadow-sm transition hover:border-sky-200 hover:bg-sky-50/35">
    <div className="relative h-20 overflow-hidden rounded-lg ring-1 ring-black/5" aria-label={`${theme.name}主题前后效果对比，拖动滑杆查看变化`}>
    ...
    <button type="button" onClick={onApply} className="mt-1.5 w-full rounded-md bg-slate-100 px-1 py-1 text-[10px] font-semibold text-slate-600 hover:bg-sky-100 hover:text-sky-700" aria-label={`应用${theme.name}主题与匹配滤镜`} title={`${theme.name}：${theme.note}，滑杆可比较应用前后`}>套用 · {theme.name}</button>
  </div>;
}
    ...
    <aside className="mobile-drawer-panel relative z-10 flex h-full w-[min(85vw,320px)] flex-col overflow-y-auto bg-[#f8fafb] px-3 pb-4 pt-4 shadow-[12px_0_35px_rgba(36,54,69,0.08)] lg:w-[300px] lg:shadow-none">
      ...
      <div className="mt-4 border-t border-slate-200 pt-3">
        ...
        <div className="mb-2 rounded-xl bg-white p-2 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-2">{BUILTIN_AI_THEMES.map(theme => <ThemeComparisonCard key={theme.id} theme={theme} before={currentAppearance} position={themeComparePositions[theme.id] ?? 50} onPositionChange={value => setThemeComparePositions(previous => ({ ...previous, [theme.id]: value }))} onApply={() => applyBuiltInTheme(theme)} />)}</div>
