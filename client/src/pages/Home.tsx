...
export default function Home() {
  ...
  const messages = useMemo<StreamMessage[]>(() => activeConversation?.messages.map(message => ({ role: message.role, content: message.content, attachments: message.attachments })) ?? [], [activeConversation]);
  ...
  async function sendMessage(payload: { text: string; attachments: Attachment[] }) {
    ...
    setIsStreaming(true);
    try {
      const response = await fetch(modelEndpoint(activeAI.baseUrl), { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${activeAI.apiKey}` }, body: JSON.stringify({ model: activeAI.model, stream: true, messages: [{ role: "system", content: `你是 ${activeAI.name}，一个准确、友善、简洁的助手。` }, ...history, { role: "user", content: buildContent({ content: text, attachments }) }] }) });
