...
function GenerationDetails({ generation }: { generation: NonNullable<LocalAsset["generation"]> }) {
  const serialized = JSON.stringify(generation.parameters, null, 2);
  return <section className="mt-4 rounded-xl border border-violet-100 bg-violet-50/60 p-3 text-xs text-slate-600"><div className="flex items-center gap-2 font-bold text-violet-800"><Sparkles className="size-3.5" />生成来源</div><dl className="mt-2 grid gap-2"><div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2"><dt className="text-slate-400">类型</dt><dd>{generation.capability}</dd></div><div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2"><dt className="text-slate-400">模型</dt><dd className="break-all font-semibold text-slate-700">{generation.model || "未记录"}</dd></div><div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2"><dt className="text-slate-400">端点</dt><dd className="break-all">{generation.endpoint || "未记录"}</dd></div>{generation.providerTemplateId && <div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2"><dt className="text-slate-400">模板</dt><dd>{generation.providerTemplateId}</dd></div>}<div><dt className="text-slate-400">提示词</dt><dd className="mt-1 whitespace-pre-wrap rounded-lg bg-white/80 p-2 leading-5 text-slate-700">{generation.prompt || "未记录"}</dd></div><details className="rounded-lg bg-white/80 p-2"><summary className="cursor-pointer font-semibold text-slate-600">查看实际请求参数</summary><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap break-all text-[10px] leading-4 text-slate-500">{serialized}</pre></details></dl><p className="mt-2 text-[10px] leading-4 text-slate-400">来源记录不包含 API Key；文件与元数据只保存在当前浏览器。</p></section>;
}
  ...
  return <main className="min-h-dvh bg-[#f6f8fb] text-slate-800">
    ...
    <div className="mx-auto grid max-w-7xl gap-4 p-4 sm:p-6 lg:grid-cols-[20rem_minmax(0,1fr)_minmax(22rem,0.9fr)]">
      ...
      <aside className="min-w-0 rounded-2xl bg-white p-3 shadow-sm lg:sticky lg:top-20 lg:max-h-[calc(100dvh-6rem)] lg:overflow-y-auto">{!selected ? <div className="grid min-h-64 place-items-center p-6 text-center"><File className="size-9 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">选择一个素材查看</p><p className="mt-1 text-xs leading-5 text-slate-400">图片、音频、视频和常见文档会在这里预览。</p></div> : <><div className="flex items-start gap-2"><span className={`grid size-9 shrink-0 place-items-center rounded-xl ${kindAccent[selected.kind]}`}>{(() => { const Icon = kindIcons[selected.kind]; return <Icon className="size-4" />; })()}</span><div className="min-w-0 flex-1">{editing ? <input value={draftName} onChange={event => setDraftName(event.target.value)} className="h-8 w-full rounded-lg border border-slate-200 px-2 text-sm font-semibold outline-none focus:border-sky-300" /> : <p className="truncate text-sm font-bold text-slate-700">{selected.name}</p>}<p className="mt-0.5 text-[11px] text-slate-400">{assetKindLabel(selected.kind)} · {formatAssetSize(selected.size)} · {formatDate(selected.updatedAt)}</p></div><button type="button" onClick={() => setEditing(value => !value)} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-100" aria-label="编辑素材信息"><Pencil className="size-3.5" /></button><button type="button" onClick={removeSelected} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-500" aria-label="删除本机素材"><Trash2 className="size-3.5" /></button></div>
        ...
        <div className="mt-4"><AssetPreview asset={selected} objectUrl={selectedUrl} /></div>{selected.generation && <GenerationDetails generation={selected.generation} />}
